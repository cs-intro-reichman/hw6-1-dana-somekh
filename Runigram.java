...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		int lenRows = image.length;
		int lencol = image[0].length;
		Color[][] newImage = new Color[lenRows][lencol];
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				newImage[i][j] = image[i][lenRows - 1 - j];
			}
		}
		return newImage;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				newImage[i][j] = image[lenRows -i -1][j];
			}
		}
		return newImage;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {	
				int oldI = i * imageHeight / height;
				int oldJ = j * imageWidth / width;
				newImage[i][j] = image[oldI][oldJ];
			}
		}
		return newImage;
	}
